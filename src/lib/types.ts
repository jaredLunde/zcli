...
> = K extends [string, ...infer Rest]
  ? K[0] extends keyof O
    ? O[K[0]] extends Record<string, unknown>
      ? Rest extends [string] ? O[K[0]][Rest[0]]
      : Rest extends [string, ...string[]] ? NestedValue<O[K[0]], Rest>
      : O[K[0]]
    : O[K[0]]
  : never
  : never;
...
export type Join<Strings extends Readonly<Array<string>>> = Strings extends []
  ? ""
  : Strings extends readonly [string] ? `${Strings[0]}`
...
type SimpleMerge<Destination, Source> = {
  [Key in keyof Destination | keyof Source]: Key extends keyof Source
    ? Source[Key]
    : Key extends keyof Destination ? Destination[Key]
    : never;
};
